...
const Index = () => {
  ...
  const handleLike = async (catchId: string) => {
    ...
    if (alreadyLiked) {
      // Logic for unliking could go here, but for now we just return
      return;
    }
    ...
    if (!error) {
      // 2. REFRESH WITHOUT LOADING SPINNER
  ...
  const handleSendComment = async (itemId: string, type: string) => {
    ...
    try {
      ...
      fetchUnifiedFeed(false); // Refresh without spinner
            ...
            {item.itemType === 'ACTIVITY' ? (
              <div className="p-8 flex flex-col items-center text-center">
                <div className="bg-black px-6 py-6 rounded-[30px] border border-primary/30 flex flex-col items-center gap-3 shadow-2xl w-full">
                  ...
                  <span className="text-xl font-black italic uppercase tracking-tighter text-white uppercase">Unlocked {item.content}</span>
                </div>
              </div>
            ) : (
